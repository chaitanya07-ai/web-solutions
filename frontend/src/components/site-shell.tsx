...
function Navbar() {
  ...
  useEffect(() => { let previous = window.scrollY; const scroll = () => { setScrolled(window.scrollY > 35); setDown(window.scrollY > previous && window.scrollY > 240); previous = window.scrollY; }; scroll(); window.addEventListener('scroll', scroll, { passive: true }); return () => window.removeEventListener('scroll', scroll); }, []);
