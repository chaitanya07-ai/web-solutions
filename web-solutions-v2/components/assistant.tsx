...
import { Instagram, MessageCircle, Phone, Send, Sparkles, X } from "lucide-react";
...
const RULES: { match: RegExp; reply: string; contact?: boolean; book?: boolean }[] = [
  {
...
function answer(text: string): { reply: string; contact?: boolean; book?: boolean } {
...
export default function Assistant() {
  ...
  const send = (text: string) => {
    ...
    setTimeout(() => {
      setMessages((m) => [
        ...m,
        { id: idRef.current++, role: "bot", text: a.reply, contact: a.contact, book: a.book },
      ]);
      setTyping(false);
    }, 750 + Math.random() * 450);
  };
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-5 right-5 z-[85] flex h-14 w-14 items-center justify-center rounded-full bg-ink text-gold shadow-lift transition-colors hover:bg-gold hover:text-ink"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 z-[85] flex max-h-[70vh] w-[calc(100vw-2rem)] max-w-sm flex-col overflow-hidden rounded-3xl border border-line bg-white shadow-lift sm:right-5"
            role="dialog"
